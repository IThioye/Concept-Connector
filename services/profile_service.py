...
class ProfileService:
    ...
    def get_profile(self, session_id):
        ...
        return {
            ...
            "knowledge_level": (row[0] if row else "intermediate"),
    ...
    def upsert_profile(self, data):
        cur = self.conn.cursor()
        cur.execute(
            "INSERT INTO user_profiles(session_id, knowledge_level, preferred_examples) VALUES(?,?,?) "
            "ON CONFLICT(session_id) DO UPDATE SET knowledge_level=excluded.knowledge_level, preferred_examples=excluded.preferred_examples",
