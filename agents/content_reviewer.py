import json
from typing import Any, Dict

from prompts.templates import REVIEW_SYSTEM, REVIEW_USER
from services.ollama_service import ollama
from .logging_config import logger
from services.text_formatter import extract_json


def _safe_json_parse(text: str) -> Dict[str, Any]:
    """Attempt to parse LLM output into structured JSON with fallbacks."""
    parsed = extract_json(text)
    
    if parsed:
        return parsed
    
    # Fallback when even robust extraction fails
    logger.warning("Reviewer failed to find structured JSON payload; falling back to loose parsing")
    
    # A slightly smarter fallback can check for keywords if JSON failed entirely
    is_aligned = "level_alignment: false" not in text.lower()
    
    return {
        # This will only be False if the LLM output was completely unparsable AND
        # it contained text suggesting misalignment (a much safer fallback)
        "level_alignment": is_aligned, 
        "reading_level": "unknown",
        "issues": [text.strip()],
        "suggested_actions": ["Rewrite to match the requested learner level. (JSON failure)"],
        "bias_risk": "unknown",
    }


class ContentReviewer:
    async def evaluate(
        self,
        bundle: Dict[str, Any],
        level: str,
        profile: Dict[str, Any] | None = None,
        concept_a: str = "",
        concept_b: str = "",
    ) -> Dict[str, Any]:
        """Ask the reviewer LLM to analyse level-fit and potential bias signals."""

        sys = REVIEW_SYSTEM
        profile = profile or {}
        usr = REVIEW_USER.format(
            level=level,
            education_level=profile.get("education_level") or "unspecified",
            education_system=profile.get("education_system") or "unspecified",
            concept_a=concept_a or "Concept A",
            concept_b=concept_b or "Concept B",
            concept_a_knowledge=profile.get("concept_a_knowledge", 0),
            concept_b_knowledge=profile.get("concept_b_knowledge", 0),
            content=json.dumps(bundle, ensure_ascii=False),
        )
        text = await ollama.agenerate(prompt=usr, system_prompt=sys, temperature=0.2)
        logger.debug("==== RAW REVIEWER OUTPUT ====")
        logger.debug(text)
        logger.debug("==================================")

        parsed = _safe_json_parse(text)
        # ensure structure consistency
        parsed.setdefault("level_alignment", True)
        parsed.setdefault("reading_level", "unknown")
        parsed.setdefault("issues", [])
        parsed.setdefault("suggested_actions", [])
        parsed.setdefault("bias_risk", "unknown")
        
        return parsed
