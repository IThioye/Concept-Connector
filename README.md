...
|  Connection Finder   | Discovery Agent  | Finds conceptual links between two ideas using the LLM          |
| Explanation Builder | Education Agent   | Crafts level‑adapted explanations plus tailored analogies       |
|  Content Reviewer    | Quality Agent     | Verifies level alignment and surfaces actionable refinements    |
| Bias Monitor        | Fairness Agent   | Detects and flags bias or cultural imbalance in results         |
|  Feedback Adapter    | Tool      | Turns learner feedback into prompt guidance for future outputs  |
...
flowchart TD
    ...
    I -->|No - aligned and no bias| L[Results packaged and persisted]
    I -->|Yes - misaligned or biased| J{Max retries<br/>exceeded?}
    
    J -->|Yes| L
    J -->|No| K[Compose mitigation guidance]
    K -->|Reuse connections<br/>Regenerate narrative| E
...
4. **Bias review & Content verification** – BiasMonitor flags bias concerns while ContentReviewer checks that explanations match the requested knowledge level and highlights refinement opportunities.
6. **Mitigation pass** – If reviewer or verification checks fail, the orchestrator re-prompts the necessary agents with corrective guidance before finalizing the response.
7. **Delivery & persistence** – Final artifacts and applied mitigations are stored via the memory service and surfaced in the UI alongside a feedback form for continuous improvement.
...
* Sections for explanations and analogies with styled Markdown output.
